...
c_data_transmission_buffer::c_data_transmission_buffer(int buffer_size, int interval_in_milisec):
		...
		,m_reference_time(chrono::steady_clock::now())
{}

void c_data_transmission_buffer::update_sent_buffer(size_t data_size){
...
void c_data_transmission_buffer::update_read_buffer(size_t data_size){
	auto now = chrono::steady_clock::now();
	long units = chrono::duration<double, milli>(now - m_reference_time).count() / m_interval;
...
string c_data_transmission_buffer::get_data_buffer_as_js_str(string var){
	ostringstream out;
...
string c_data_transmission_buffer::get_packets_buffer_as_js_str(string var){
	ostringstream out;
	update_read_buffer(0);
	update_sent_buffer(0);
...
string c_data_transmission_buffer::get_charts_as_js_str(string var, bool display_big_chart){
	ostringstream out;
...
c_transmission_stats::c_transmission_stats() : m_size_of_sent_data(0)
		, m_size_of_read_data(0)
		, m_number_of_sent_packets(0)
		, m_number_of_read_packets(0)
		, m_connection_time(chrono::steady_clock::now())
		, m_data_buffer(120, 1000)
{}

void c_transmission_stats::update_sent_stats(size_t size_of_sent_data){
	m_data_buffer.update_sent_buffer(size_of_sent_data);
	m_size_of_sent_data += size_of_sent_data;
	m_number_of_sent_packets++;
}

void c_transmission_stats::update_read_stats(size_t size_of_read_data){
	m_data_buffer.update_read_buffer(size_of_read_data);
	m_size_of_read_data += size_of_read_data;
	m_number_of_read_packets++;
}

size_t c_transmission_stats::get_size_of_sent_data() const{
	return m_size_of_sent_data;
}

size_t c_transmission_stats::get_size_of_read_data() const{
	return m_size_of_read_data;
}

long c_transmission_stats::get_number_of_sent_packets() const{
	return m_number_of_sent_packets;
}

long c_transmission_stats::get_number_of_read_packets() const{
	return m_number_of_read_packets;
}

string c_transmission_stats::get_connection_time() const{
	ostringstream out;
	auto now = chrono::steady_clock::now();
...
void c_transmission_stats::reset_connection_time(){
	m_connection_time = chrono::steady_clock::now();
}
