...
std::pair<std::string, int> tunserver_utils::parse_ip_string(const std::string &ip_string) {
	...
	if (ip_string.size()<(1+1)*4) pfp_throw_error(runtime_error(join_string_sep("Invalid (too small) IP size",ip_string.size())));
	if (ip_string.size()>(4*4+1+4)) pfp_throw_error(runtime_error(join_string_sep("Invalid (too big) IP size",ip_string.size())));
	/*
...
//		throw std::invalid_argument("bad format of input ip address");
	...
	*/
	...
	if (pos == string::npos) pfp_throw_error( runtime_error( join_string_sep("Invalid IP format (char ':')", ip_string)));
	std::string ip = ip_string.substr(0, pos);
	if (ip.size()<(1+1)*4) pfp_throw_error(runtime_error(join_string_sep("Invalid (too small) IP size (ip part)",ip.size())));
	std::string port = ip_string.substr(pos+1);
	if (ip.size()<1) pfp_throw_error(runtime_error(join_string_sep("Invalid (too small) IP size (port part)", port.size())));
	pfp_dbg1("Parsing IP as strings: " << ip <<  " port string: " << port);
	int port_int = std::stoi(port);
	if (port_int<=0) pfp_throw_error(runtime_error(join_string_sep("Invalid port", port_int)));
	if (port_int>65535) pfp_throw_error(runtime_error(join_string_sep("Invalid port", port_int)));
	...
	if (ec) pfp_throw_error(runtime_error(join_string_sep("Invalid address (boost detected)", ip, ec)));
	...
	if (!( is_good_class && !is_bad_class ))
		pfp_throw_error(runtime_error(join_string_sep("Invalid address (not a normal ip-class of address)",ip_boost)));

	return std::make_pair(std::move(ip), port_int);

	} catch(std::exception &ex) {
		pfp_throw_error(std::invalid_argument( join_string_sep( ex.what() , mo_file_reader::gettext("L_bad_format_of_input_ip_address")) ));
	}
}
