...
void c_user::recieve_token (c_token &token, size_t amount) {
    ...
    if(m_edsigner.verify(token.m_chainsign.back().m_signed_msg,
                         token.m_chainsign.back().m_msg,
                         token.m_chainsign.back().m_msg.length(),
                         token.m_chainsign.back().m_signer_pubkey)) {
