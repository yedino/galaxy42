...
session::session(tcp::socket socket, const c_tunserver & tunserver) : socket_(move(socket))
...
void session::do_read()
{
	...
	socket_.async_read_some(boost::asio::buffer(data_, max_length)
			, [this, self](boost::system::error_code ec, size_t length){if (!ec) do_write(length);});
}
...
void session::do_write(size_t length)
{
	...
	boost::asio::async_write(socket_, boost::asio::buffer(m_reply, m_reply.size())
...
string c_httpdbg_raport::generate(string url) {
	...
	for(auto it = m_target.m_tunnel.begin(); it != m_target.m_tunnel.end(); it++)
	{
		...
		if(it->second->m_stream_crypto_ab != nullptr)
		{
		...
		if(it->second->m_stream_crypto_final != nullptr)
		{
