...
struct NODE {
	...
	bool flagis(vector<t_flag> ftab) { for(auto o:ftab) if(flag.count(o)) return 1; return 0; }
	...
	void jumpaway() { move+=P(randm11(),randm11())*5; }
};
...
bool c_simulation::routingdemo_main() {
	...
	auto nnn = world->node_any();
	nnn->flagadd({e_mark});
	for(auto const &ptr : world->nodes_close(*nnn,5.0,10)) { ptr->flagadd({e_red}); }
	...
	while (!m_goodbye && !close_button_pressed) {	try {
		...
		for(auto obj1 : world->nodes) { for(auto obj2 : world->nodes) { if (obj1==obj2) continue; // avoid nodes collision
			auto p1=obj1->p, p2=obj2->p; auto d=obj1->p.dist(obj2->p);  if (fabs(d)<2) { obj1->jumpaway(); continue; }
			if (d>30) continue; auto s=10/clamp(d,5,30); obj1->move += (p1-p2).norm() * pow(s,2);
		} }
		for(auto obj1 : world->nodes) { for(auto obj2w : obj1->peer) { auto obj2=obj2w.lock(); check( obj1 != obj2); // pull to peers
			auto p1=obj1->p, p2=obj2->p; auto d=obj1->p.dist(obj2->p);  if (fabs(d)<2) { obj1->jumpaway(); continue; }
			if (d>50) continue; auto s=10/clamp(d,5,30); obj1->move += - (p1-p2).norm() * pow(s,3)*1;
		} }
