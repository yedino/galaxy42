...
TEST(time_utils, check_readable_time_regex) {
	...
	std::string formated = time_t_to_readable(gen_exact_date(1993,6,1,1,1,1));
	pfp_mark("time: " << formated << ", formated_length: " << formated.length());
...
TEST(time_utils, exact_data_check) {
	...
	pfp_mark("time Europe/Warsaw: " << formated << ", formated_length: " << formated.length());
	EXPECT_EQ(formated, "2017-10-21T12:01:03+02:00");

	formated = time_t_to_readable(gen_exact_date(2017,10,1,0,0,0), "Asia/Pyongyang");
	pfp_mark("time Asia/Pyongyang: " << formated << ", formated_length: " << formated.length());
	...
	pfp_mark("time Pacific/Galapagos: " << formated << ", formated_length: " << formated.length());
