 ...
 */
#define pfp_DEAD_RETURN_MAYBE() do { if (volatile_always_true<0>()) { pfp_erro("Reached unexpected end of function (DEAD_RETURN), aborting.");  std::abort(); } } while (0)
 ...
 */
#define pfp_DEAD_RETURN() do { { pfp_erro("Reached unexpected end of function (DEAD_RETURN), aborting.");  std::abort(); } } while (0)

#define pfp_TODOCODE { std::stringstream oss; oss<<"Trying to use a not implemented/TODO code, in " \
