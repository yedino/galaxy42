...
void c_multisign::print_signatures() const {
	pfp_info("Printing all sinatures in c_multisign:");
...
void c_multikeys_pub::multi_sign_verify(const std::vector<string> &signs,
	...
	pfp_dbg2("pubkeys: " << amount_of_pubkeys << " signatures: " << signs.size());
		...
		case e_crypto_system_type_NTRU_sign: {
			...
			for(size_t i = 0; i < amount_of_pubkeys; ++i) {
				...
				if (!ntrupp::verify(signs.at(i), msg, pubkey)) {
					pfp_throw_error( std::invalid_argument("Ntru sign verify: fail") );
				}
			}
			#else
				pfp_throw_error( invalid_argument_in_version("NTru signature (verify) not enabled") );
			#endif
			break;
		}
		default: pfp_throw_error( std::invalid_argument("sign type not supported") );
	}
}
...
void c_multikeys_pub::multi_sign_verify(const c_multisign &all_signatures,
	...
	if (all_signatures.get_count_of_systems() != pubkeys.get_count_of_systems()) {
		pfp_throw_error( std::invalid_argument("count of systems in c_multikeypub and c_multisign different!") );
	}
...
std::vector<string> c_multikeys_PRV::multi_sign(const string &msg, t_crypto_system_type sign_type) {
	...
	switch(sign_type) {
		...
		case e_crypto_system_type_NTRU_sign: {
			...
			#else
				pfp_throw_error( invalid_argument_in_version("NTru signature (signing) not enabled") );
			#endif
			break;
		}
		default: pfp_throw_error( std::runtime_error("sign type not supported") );
	}
...
void c_multikeys_PAIR::debug() const {
	...
	for (size_t ix=0; ix<m_pub.m_cryptolists_general.size(); ++ix) { // for all key type (for each element)
		...
		if (size_pub || size_PRV) {
			if (size_pub != size_PRV) pfp_warn("Not matching count of pub and prv keys! " << size_pub << " vs " << size_PRV);
			for (size_t key_nr=0; key_nr<std::min(size_pub , size_PRV); ++key_nr) {
				const auto name = t_crypto_system_type_to_name(ix);
				pfp_info("pub " + name + " #" << key_nr << "/" << size_pub << " "
					<< to_debug( pubkeys_of_this_system.at(key_nr)) );
				pfp_info("PRV " + name + " #" << key_nr << "/" << size_PRV << " "
...
void c_multikeys_PAIR::generate(t_crypto_system_count cryptolists_count, bool will_asymkex) {
	pfp_info("Generating from cryptolists_count");
	for (size_t sys=0; sys<cryptolists_count.size(); ++sys) { // all key crypto systems
		...
		if (how_many > 0) {
			pfp_info("Generate keys " << t_crypto_system_type_to_name(sys_enum) << " in amount: "<<how_many);
			if ( will_asymkex || false==t_crypto_system_type_is_asymkex(sys_enum) ) {
				this->generate(sys_enum, how_many);
			} else pfp_dbg1("Skipping because this is asymkex, and we do not want to do that "
...
std::pair<sodiumpp::locked_string, string> c_multikeys_PAIR::generate_x25519_key_pair() {
	...
	//pfp_info("Random data size=" << (rnd.size()) );
	//pfp_info("Random data=" << to_debug_locked(rnd) );
...
std::pair<sodiumpp::locked_string, string> c_multikeys_PAIR::generate_nrtu_encrypt_key_pair() {
		...
		return keypair;
	#else
		pfp_throw_error( invalid_argument_in_version("NTru encryption (keygen) not enabled") );
	#endif
}
...
std::pair<sodiumpp::locked_string, string> c_multikeys_PAIR::generate_nrtu_sign_key_pair() {
		...
		return keypair;
	#else
		pfp_throw_error( invalid_argument_in_version("NTru signature (keygen) not enabled") );
	#endif
}
...
std::pair<sodiumpp::locked_string, string> c_multikeys_PAIR::generate_sidh_key_pair() {
	...
	#else
		pfp_throw_error( invalid_argument_in_version("SIDH (keypair gen) not enabled") );
	#endif
}
...
void c_multikeys_PAIR::generate(t_crypto_system_type crypto_system_type, int count) {
	...
	switch (crypto_system_type)
	{
		...
		case e_crypto_system_type_NTRU_sign:
		{
			...
			#else
				pfp_throw_error( invalid_argument_in_version("SIDH (generate) not enabled") );
			#endif
			break;
		}
	...
	pfp_info("Serialized pubkeys: [" << to_debug(serialized) << "]");
}
	 ...
	 const c_crypto_system::t_PRVkey & PRVkey)
{
	...
	pfp_info("Adding PRIVATE KEY: (" << t_crypto_system_type_to_name(crypto_type) << ") " << to_debug_locked(PRVkey));
	m_PRV.add_PRIVATE(crypto_type, PRVkey);
	//pfp_note("ADD PRIVATE KEY: RESULT IS: " << to_debug(m_PRV.serialize_bin()));
}
