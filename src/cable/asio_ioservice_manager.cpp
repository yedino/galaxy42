...
void c_asioservice_manager::resize_to_at_least(size_t size_) {
	pfp_note("Resizing to size_=" << size_ );
	guard_LOCK(m_mutex);
	guard_inv;

	pfp_note("Resizing to size_" << size_ << " now size="<<size()<<" / capacity="<<capacity()
		<< "; other arrays: ..._threads=" << m_ioservice_threads.size() << " ..._works=" << m_ioservice_idle_works.size() );
	if (m_stop) { pfp_note("Will NOT resize, because m_stop"); return ; }

	if (size_ <= m_size) {
		pfp_dbg1("We already have requested size");
...
void c_asioservice_manager::stop_all_threadsafe() {
	...
	pfp_note("Stopping all in SIOM, m_size="<<m_size);
	for (size_t i = 0; i < m_size; i++) stop_ioservice(i);
}


void c_asioservice_manager::run_ioservice(size_t index) {
	pfp_info("run_ioservice index="<<index);
	guard_LOCK(m_mutex); // lock again - why not
	// guard_inv;
	pfp_note("Starting ioservice index="<<index<<" (from our array size=" << m_ioservice_array.size()<<")");
	...
	m_ioservice_threads.emplace_back(
		std::thread([&io_service_ref = m_ioservice_array.at(index) , index] {
			pfp_note("Starting in thread " << std::this_thread::get_id() << ", ioservice index="<<index
				<< " io_service_ref="<<static_cast<void*>(&io_service_ref));
			io_service_ref.run();
			pfp_note("Ok, service started for index="<<index);
		})
	);
}
...
void c_asioservice_manager::stop_ioservice(size_t index) {
	...
	pfp_note("Stopping ioservice index="<<index<<" (from our array size=" << m_ioservice_array.size()<<")");
