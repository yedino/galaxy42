...
class arrayvector {
	...
	public:
		...
		size_t max_size() const; ///< the maximum possible size (it is set by TMax)
		size_t capacity() const; ///< same as max_size()
};
...
T & arrayvector<T,TMax>::at(size_t ix) {
	...
	if (ix >= m_current_size) throw std::out_of_range(oss.str()); return m_array[ix];
}
template <typename T, size_t TMax>
...
template <typename T, size_t TMax>
size_t arrayvector<T,TMax>::max_size() const { return TMax; }

template <typename T, size_t TMax>
size_t arrayvector<T,TMax>::capacity() const { return TMax; }
