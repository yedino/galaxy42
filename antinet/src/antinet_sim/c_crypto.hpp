...
public:
	...
	rgpr_signed_msg<long_type> rgpr_sign (const string &msg) const {
		long_type Private_key[256 + 8];
		long_type hashed_msg = sha256<long_type>(msg);

		for (long_type &in : Private_key)
			in = rd_gen.get_random(33); // (256+8)/8

		rgpr_signed_msg<long_type> signature;
		...
		for (short i = 0; i < 256 + 8; ++i) {
			...
			signature.public_key ^= sha256<long_type>((string)hashed_private_key);
		}
		return signature;
	}

	bool rgpr_verify_sign (const string &msg, const rgpr_signed_msg<long_type> &sign) const {
		long_type hashed_msg = sha256<long_type>(msg), tmp;
		long_type key = 0;
		for (short i = 0; i < 256 + 8; ++i) {
			if (hashed_msg & (1 << i))
				tmp = sha256<long_type>((string)sign.Signature[i]);
			else
				tmp = sign.Signature[i];

			key ^= sha256<long_type>((string)tmp);
		}
		return (key == sign.public_key);
	}
};
