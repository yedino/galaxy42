...
c_chainsign_element::c_chainsign_element(const std::string signed_msg, const std::string msg, const std::string signer_pubkey) :
    m_signed_msg(signed_msg),
    m_msg(msg),
    m_signer_pubkey(signer_pubkey) {
}

c_token::c_token (long long pss) :
    m_password(pss),
    id(token_id_generator::generate_id()) {
}
