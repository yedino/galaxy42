...
c_allegro_layer::c_allegro_layer (BITMAP *frame, t_layer_nr layer_nr) : m_frame(frame), m_layer_nr(layer_nr) { }
...
c_drawtarget_allegro::c_drawtarget_allegro (BITMAP *frame) : c_drawtarget(e_drawtarget_type_allegro) {
	...
	for (int nr = (int)e_layer_nr_bgr; nr < (int)e_layer_nr_top; ++nr) {
		m_layer.push_back(c_allegro_layer(frame, static_cast<t_layer_nr>(nr)));
	}
}
