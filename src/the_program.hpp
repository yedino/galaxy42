...
class c_the_program {
	public:
		...
		virtual void take_args(const std::string & _argt_exec , const std::vector<std::string> & _argt); ///< sets exec-name, and rest of args
		...
		virtual bool check_and_remove_special_cmdline(const std::string & name); ///< checks if given command-line argument is given, if yes then remove it and return true
	...
	protected:
		...
		///@{
		std::vector<std::string> argt; ///< the arguments with which the program is running, except for program name (see argt_exec)
		std::string argt_exec; ///< the name of running binary program (like from argv[0])
		///@}
		...
		std::shared_ptr<boost::program_options::options_description> m_boostPO_desc; ///< description/definition of the possible options, for parsing
		///@}

		/// Dirs/path options for the program
		/// @{
		std::string m_install_dir_base; ///< here we will find main dir like "/usr/" that contains our share dir
		...
		std::string config_default_myname;
};
