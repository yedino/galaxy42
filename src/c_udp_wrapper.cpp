...
c_udp_wrapper_openbsd::c_udp_wrapper_openbsd(const int listen_port)
:
m_socket(socket(AF_INET, SOCK_DGRAM, 0)) {
    //int state = 1;
...
void c_udp_wrapper_openbsd::send_data(const c_ip46_addr &dst_address, const void *data, size_t size_of_data) {
    if (m_disabled) {
	pfp_dbg4n("disabled socket");
	return;
    }
    auto dst_ip4 = dst_address.get_ip4(); // ip of proper type, as local variable
...
size_t c_udp_wrapper_openbsd::receive_data(void *data_buf, const size_t data_buf_size, c_ip46_addr &from_address) {
    if (m_disabled) {
	pfp_dbg4n("disabled socket");
	return 0;
    }
...
int c_udp_wrapper_openbsd::get_socket() {
    if (m_disabled) {
	pfp_dbg4n("disabled socket");
	return 0;
    }
