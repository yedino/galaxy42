...
void c_rpc_bitwallet::set_wallet(std::string username, std::string password, std::string address,int port) {
...
std::string c_rpc_bitwallet::get_account_address(const std::string& account_name) {
    std::string address = c_rpc_api->getaccountaddress(account_name);
    return address;
}

std::string c_rpc_bitwallet::sendfrom(const std::string& fromaccount, const std::string& tocoinaddress, double amount) {
    std::string txid = c_rpc_api->sendfrom(fromaccount, tocoinaddress, amount);
    return txid;
}
